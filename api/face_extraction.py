...
def resize_image(img, scale_percent): #hàm resize ảnh
    width = int(img.shape[1] * scale_percent / 100)
    height = int(img.shape[0] * scale_percent / 100)
    dim = (width, height)
    return cv2.resize(img, dim)

def detect_face(img, scale_percent=50, expand_ratio=0.2):  #hàm phát hiện và điều chỉnh phần cắt khuôn mặt
...
def process_student_id(img_path): #hàm trả về ảnh đã cắt dưới dạng base64
    try:
